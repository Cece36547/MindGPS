...
export default function MoodCheckIn({
  ...
  // (Andy) keep the same form logic and let it work inside journal too
  const content = (
    <>
      <button
        type="button"
      ...
      <div className={`rounded-3xl border border-violet-200 bg-white/90 p-6 shadow-xl ${isInline ? "mx-auto max-w-3xl" : ""}`}>
        ...
        <form onSubmit={handleSave} className="mt-6 space-y-5">
          <div>
            ...
            <p className="mb-3 text-xs text-[#6b6485]">Pick any relevant tags.</p>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <MindButton type="submit" className="h-12 w-full px-8 sm:w-auto" fullWidth={false}>
              Save Entry
            </MindButton>
  ...
  if (isInline) {
    return <div className="p-4 sm:p-6 lg:p-8">{content}</div>;
  }

  return (
    <main className="min-h-screen bg-[#ede8ff] text-[#2f1d69]">
      <div className="mx-auto max-w-3xl p-6 md:p-10">{content}</div>
    </main>
  );
}
